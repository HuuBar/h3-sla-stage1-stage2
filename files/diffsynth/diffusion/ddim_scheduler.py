...
class DDIMScheduler():
    ...
    def __init__(self, num_train_timesteps=1000, beta_start=0.00085, beta_end=0.012, beta_schedule="scaled_linear", prediction_type="epsilon", rescale_zero_terminal_snr=False):
        ...
        self.training = False
        self.sigma_weight_cap = None  # >0 时 FlowMatchSFT loss 按 sigma 加权 (min-SNR 风格)
    ...
    def sigma_at(self, timestep):
        """timestep (0..999) 处的噪声水平 sigma = sqrt(1 - alpha_cumprod[t])。"""
        timestep_id = max(min(int(timestep.flatten().tolist()[0]), len(self.alphas_cumprod) - 1), 0)
        return math.sqrt(max(1.0 - self.alphas_cumprod[timestep_id], 0.0))

    def sigma_weight(self, timestep, cap=10.0):
        """按 sigma 加权 (min-SNR 风格): weight = min(1/sigma^2, cap), 低噪端权重大。
        默认 cap=10; cap<=0 表示不加权 (恒 1)。"""
        if cap is None or cap <= 0:
            return 1.0
        s = self.sigma_at(timestep)
        if s < 1e-6:
            return float(cap)
        return min(1.0 / (s * s), float(cap))
