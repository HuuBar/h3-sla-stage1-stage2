...
class DiffusionTrainingModule(torch.nn.Module):
    ...
    def add_lora_to_model(self, model, target_modules, lora_rank, lora_alpha=None, upcast_dtype=None, lora_bias="none"):
        if lora_alpha is None:
            lora_alpha = lora_rank
        if isinstance(target_modules, list) and len(target_modules) == 1:
            # Unwrapping to a string makes PEFT treat it as a regex, which is the
            # only way to pass a pattern through the comma-separated CLI flag.
            target_modules = target_modules[0]
        lora_config = LoraConfig(r=lora_rank, lora_alpha=lora_alpha, target_modules=target_modules, bias=lora_bias)
    ...
    def split_pipeline_units(
        ...
        elif task.startswith("sft:train"):
            # 覆盖 "sft:train" 与 "sft:train_align" (阶段二 teacher 对齐)
