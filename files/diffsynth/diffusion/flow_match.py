...
class FlowMatchScheduler():
    ...
    def sigma_weight(self, timestep, cap=10.0):
        """min-SNR 风格 sigma 加权: weight = min(1/sigma^2, cap), 低噪端权重大。
        FlowMatch 的 sigma 直接是噪声水平 (sigmas 属性)。"""
        if isinstance(timestep, torch.Tensor):
            timestep = timestep.to(self.timesteps.device)
        timestep_id = torch.argmin((self.timesteps - timestep).abs())
        sigma = self.sigmas[timestep_id].float().clamp(min=1e-3)
        w = (1.0 / (sigma * sigma)).clamp(max=float(cap))
        return w
