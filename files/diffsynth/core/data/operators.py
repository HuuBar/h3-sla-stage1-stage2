...
class LoadAudioWithTorchaudio(DataProcessingOperator, FrameSamplerByRateMixin):
    ...
    def __init__(self, num_frames=121, time_division_factor=8, time_division_remainder=1, frame_rate=24, fix_frame_rate=True):
        FrameSamplerByRateMixin.__init__(self, num_frames, time_division_factor, time_division_remainder, frame_rate, fix_frame_rate)
        import torchaudio
        self._torchaudio_load = torchaudio.load
        import soundfile
        self._soundfile = soundfile

    def _load_audio(self, data: str):
        """torchaudio 优先, torchcodec 不可用(CUDA 构建 .so 在 NPU 环境加载失败)时回退 soundfile."""
        try:
            return self._torchaudio_load(data)
        except Exception:
            # soundfile.read -> (samples, channels) float; 转成 (channels, samples) tensor
            audio, sr = self._soundfile.read(data, dtype="float32")
            waveform = torch.from_numpy(audio.T if audio.ndim > 1 else audio[None])
            return waveform, sr
    ...
    def __call__(self, data: str):
        try:
            ...
            # 按视频帧率对齐目标采样数 (与官方逻辑一致: num_frames / frame_rate * sr)
            target_samples = int(self.num_frames / self.frame_rate * sample_rate)
