...
def _materialize_state_dict(model, state_dict):
    """Pull the state dict into memory, skipping values that no rank will read.

    `_load_state_dict_into_zero3_model` guards the actual copy with
    `if torch.distributed.get_rank() == 0`, so every other rank joins the gather
    but never looks at a parameter value. Materializing them anyway costs one
    full copy of the model in host memory per rank, and at sixteen ranks to a
    node that is the largest allocation in the job by a wide margin.

    Buffers are kept everywhere: ZeRO-3 leaves them unpartitioned and the loader
    copies those on every rank.
    """
    if not is_deepspeed_zero3_enabled():
        return {i: state_dict[i] for i in state_dict}
    import torch.distributed as dist

    if not dist.is_initialized() or dist.get_rank() == 0:
        return {i: state_dict[i] for i in state_dict}
    buffer_names = {name for name, _ in model.named_buffers()}
    unread = torch.empty(0)
    return {i: (state_dict[i] if i in buffer_names else unread) for i in state_dict}


# SLA 新增参数白名单: 原版 checkpoint 没有这些 key, 加载时允许缺失.
# 仅当 missing keys 全部命中白名单时才放宽 strict, 避免掩盖真正的加载错误.
SLA_MISSING_KEY_PREFIXES = (".proj_l.weight", ".proj_l.bias")


def _load_state_dict(model, state_dict, assign=True):
    """load_state_dict with SLA-aware strict fallback."""
    try:
        model.load_state_dict(state_dict, assign=assign)
    except RuntimeError as e:
        missing = _parse_missing_keys(str(e))
        if missing is not None and missing and all(
            any(m.endswith(p) for p in SLA_MISSING_KEY_PREFIXES) for m in missing
        ):
            model.load_state_dict(state_dict, assign=assign, strict=False)
            return
        raise


def _parse_missing_keys(msg):
    """Extract missing key names from load_state_dict RuntimeError text."""
    try:
        if "Missing key(s)" not in msg:
            return None
        block = msg.split("Missing key(s) in state_dict:", 1)[1]
        block = block.split("Unexpected key(s)", 1)[0]
        return re.findall(r"'([^']+)'|\"([^\"]+)\"", block) and [
            (a or b) for a, b in re.findall(r"'([^']+)'|\"([^\"]+)\"", block)
        ]
    except Exception:
        return None
...
def load_model(model_class, path, config=None, torch_dtype=torch.bfloat16, device="cpu", state_dict_converter=None, use_disk_map=False, module_map=None, vram_config=None, vram_limit=None, state_dict=None, quantize=None):
    ...
    if module_map is not None and quantize is None:
        ...
        if vram_config["offload_device"] != "disk":
            ...
            _load_state_dict(model, state_dict, assign=True)
        ...
        # Why is the staging device not the target device under ZeRO-3?
        # The state dict is materialized in full before it gets scattered into the
        # partitioned model, so reading it onto the accelerator would require the
        # whole unsharded model to fit on one card, which is what sharding exists
        # to avoid. Staging on the host and letting the scatter copy parameter by
        # parameter keeps `remote_device` free to be the accelerator, so the
        # resulting shards are resident where they will be used.
        staging_device = "cpu" if is_deepspeed_zero3_enabled() else device
        if state_dict is not None:
            pass
        elif use_disk_map:
            state_dict = DiskMap(path, staging_device, torch_dtype=torch_dtype)
        else:
            state_dict = load_state_dict(path, torch_dtype, staging_device)
        ...
        if state_dict_converter is not None:
            ...
            state_dict = _materialize_state_dict(model, state_dict)
