#!/bin/bash
# ============================================================
# stage1 式线性分支再训 (2026-08-25, node-17-107)
# 起点: opencomp step-156 (主干 + proj_l 都是现在训练好的权重)
# 数据: 768p 2493 cache 再过一轮 (1 epoch = 156 步)
# 方式: 与 stage1 相同 --proj-only 冻结主干 + --teacher-align 逐层对齐
#       (loss = 每层 o_sla vs o_full 全注意力 MSE, 梯度直达 proj_l)
...
CKPT=/mnt/share/c50058431/c50058431/minimax_dmd/models/train/MiniMax-H3-T2VA-SLA-768p-2493-156-opencomp/step-156.safetensors
OUT=$REPO/models/train/MiniMax-H3-T2VA-SLA-768p-2493-156-opencomp-proj
...
CKPT_LINK=/mnt/share/c50058431/c50058431/dataset/transformer_step156_opencomp_link.safetensors
...
echo "[$(date '+%F %T')] ==== SLA proj-only 再训: opencomp step-156 起点, teacher-align, lr=1e-4, 768p 2493 x1 ===="
