#!/bin/bash
# 130 机: new-cont step-309 proj 续训 (单卡版, 已验证路线)
# 数据: stage2_full 全量 4959 条 -> 单卡 4959 步 (~28h); 想子集改 DATA_BASE
set -e
REPO=/mnt/share/c50058431/c50058431/minimax_dmd
DATA_BASE=${DATA_BASE:-/mnt/share/c50058431/c50058431/dataset/ir5k_gen_latent_cache_768p_stage2_sub1000}
MODEL_BASE=/mnt/share/c50058431/c50058431/FL2VA
CKPT=/mnt/share/c50058431/c50058431/minimax_dmd/models/train/MiniMax-H3-T2VA-SLA-768p-new-cont/step-309.safetensors
OUT=$REPO/models/train/MiniMax-H3-T2VA-SLA-768p-newcont-proj-1g
CFG=$REPO/examples/minimax_h3/model_training/full/accelerate_config_single_gpu.yaml
TRAIN=$REPO/examples/minimax_h3/model_training/train.py
SAVE_STEPS=${SAVE_STEPS:-500}

[ -f "$CKPT" ] || { echo "起点不存在: $CKPT"; exit 1; }
[ -d "$DATA_BASE" ] || { echo "数据不存在: $DATA_BASE"; exit 1; }

CKPT_LINK=/mnt/share/c50058431/c50058431/dataset/transformer_newcont309_link.safetensors
ln -sf "$CKPT" "$CKPT_LINK"
MODEL_PATHS_DIT=$(python - "$CKPT_LINK" <<'PY'
import json, sys
print(json.dumps([[sys.argv[1]]]))
PY
)

export SLA_FORCE_OFF=1
export ASCEND_RT_VISIBLE_DEVICES=${NPU_IDX:-0}
cd $REPO
echo "[proj-1g] data=$DATA_BASE ($(ls $DATA_BASE/*.pth | wc -l) 条) npu=$ASCEND_RT_VISIBLE_DEVICES"

accelerate launch --config_file $CFG \
  --num_processes 1 \
  $TRAIN \
  --dataset_base_path $DATA_BASE \
  --data_file_keys "video,input_audio" \
  --extra_inputs "input_audio" \
  --height 768 --width 1344 --num_frames 124 \
  --dataset_repeat 1 \
  --model_paths "$MODEL_PATHS_DIT" \
  --processor_path "$MODEL_BASE/processor" \
  --learning_rate 1e-4 \
  --enable_model_cpu_offload \
  --num_epochs 1 \
  --remove_prefix_in_ckpt "pipe.dit." \
  --output_path $OUT \
  --trainable_models "dit" \
  --use_gradient_checkpointing \
  --find_unused_parameters \
  --use_sla \
  --sla_topk 0.05 \
  --sla_feature_map softmax \
  --sla_blkq 64 \
  --sla_blkk 64 \
  --proj-only \
  --teacher-align \
  --align-timesteps 50 \
  --save_steps $SAVE_STEPS \
  --enable_tensorboard_log \
  --task "sft:train_align"
